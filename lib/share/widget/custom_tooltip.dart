...
///
/// CustomTooltips provide text labels which help explain the function of a button or
...
///
/// CustomTooltips improve the accessibility of visual widgets by proving a textual
...
///  * [CustomTooltipTheme] or [ThemeData.tooltipTheme]
class CustomTooltip extends StatefulWidget {
  ...
  /// provided by [CustomTooltipTheme.of] will be used if a [CustomTooltipTheme] is present
  /// or specified in [ThemeData].
  ///
  /// All parameters that are defined in the constructor will
  /// override the default values _and_ the values in [CustomTooltipTheme.of].
  const CustomTooltip({
    Key key,
  ...
  ///
  /// If this property is null, then [CustomTooltipThemeData.margin] is used.
  /// If [CustomTooltipThemeData.margin] is also null, the default margin is
  ...
  /// 4.0. CustomTooltips will also default to an opacity of 90% and with the color
...
class _CustomTooltipState extends State<CustomTooltip> with SingleTickerProviderStateMixin {
  ...
  void _handleStatusChanged(AnimationStatus status) {
    if (status == AnimationStatus.dismissed) {
      _hideCustomTooltip(immediately: true);
    }
  }

  void _hideCustomTooltip({bool immediately = false}) {
    _showTimer?.cancel();
    _showTimer = null;
    if (immediately) {
      _removeEntry();
  ...
  void _showCustomTooltip({bool immediately = false}) {
    _hideTimer?.cancel();
    _hideTimer = null;
    if (immediately) {
      ensureCustomTooltipVisible();
      return;
    }
    _showTimer ??= Timer(waitDuration, ensureCustomTooltipVisible);
  }
  ...
  /// become null.
  bool ensureCustomTooltipVisible() {
    _showTimer?.cancel();
    _showTimer = null;
    if (_entry != null) {
  ...
  void _handleLongPress() {
    ...
    final bool tooltipCreated = ensureCustomTooltipVisible();
  ...
  Widget build(BuildContext context) {
    ...
    height = widget.height ?? tooltipTheme.height ?? _defaultCustomTooltipHeight;
    ...
    if (_mouseIsConnected) {
      result = MouseRegion(
        onEnter: (PointerEnterEvent event) => _showCustomTooltip(),
        onExit: (PointerExitEvent event) => _hideCustomTooltip(),
        child: result,
      );
    }
